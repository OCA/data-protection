...
class TestPartnerAnonymize(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user_eric = cls.env.ref(
            "privacy_partner_to_be_forgotten.user_eric_cartman"
        )
        ...
        cls.test_user = cls.env["res.users"].create({
            ...
            "groups_id": [
                ...
                (4, cls.env.ref("privacy_partner_to_be_forgotten.group_partner_anonymize").id),
            ],
        })
        ...
        cls.env['ir.model.access'].create({
            ...
            'perm_create': True,
            'perm_unlink': True,
            'group_id': cls.env.ref('privacy_partner_to_be_forgotten.group_partner_anonymize').id,
        })
        ...
        cls.attachment = cls.env["ir.attachment"].create({
            ...
            "res_id": cls.partner_eric.id,
            "datas": "R0lGODlhAQABAIAAAP///wAAACwAAAAAAQABAAACAkQBADs=",  # Minimal GIF
        })

    def _create_anonymize_wizard(self, partner_ids, user):
        """Helper method to create and return an anonymization wizard"""
        with Form(self.env["partner.anonymize.wizard"].with_user(user).with_context(default_partner_ids=partner_ids, active_ids=partner_ids)) as wizard:
    ...
    def _check_anonymized_partner(self, partner, original_name):
        ...
        # Check email format
        email_pattern = r"^{}_[\d\.]{{8}}_\d+@anonymized\.oca$".format(
            initials.lower()
        )
    ...
    def test_02_anonymize_child_record_only(self):
        ...
        wizard = self._create_anonymize_wizard([self.partner_butters.id], self.test_user)
        wizard.action_confirm()
    ...
    def test_03_anonymize_company_not_allowed(self):
        ...
        wizard = self._create_anonymize_wizard([self.partner_company.id], self.test_user)
        ...
        self.assertIn("Cannot anonymize the following company records", str(context.exception))
    ...
    def test_05_anonymize_deeply_nested_child(self):
        ...
        wizard = self._create_anonymize_wizard([self.partner_stanley.id], self.test_user)
        wizard.action_confirm()
    ...
    def test_06_anonymize_multiple_partners(self):
        ...
        wizard = self._create_anonymize_wizard(
            [self.partner_eric.id, self.partner_butters.id], 
            self.test_user
        )
        wizard.action_confirm()
    ...
    def test_07_validate_access_rights(self):
        ...
        # Check the error message
        self.assertIn("You don't have permission to anonymize partners", str(context.exception))
